  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-900/80">
        <div className="h-full rounded-full bg-cyan-400 transition-all duration-500" style={{ width: `${width}%` }} />
      </div>
    </div>
  );
}
...
export default function InsightsPanel({ recommendations }) {
  ...
  return (
    <section className="grid gap-4">
      <div className="surface-shell rounded-[2rem] p-5">
        ...
        <p className="mt-1 text-sm text-slate-300">Live readout of the strongest outputs, rendered as bars instead of generic charts.</p>
          ...
          {recommendations.map((item) => (
            <div key={item.productId} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Score {item.score}</p>
              </div>
